    ...
    def train(self, env, num_iterations=100, alpha=0.2, gamma=0.7, start_epsilon=0.5):
        ...
        for k in range(num_iterations):
            ...
            while not done:
                ...
                # next_walls, next_borders, next_dead_ends, next_target, next_least_explored, next_second_least_explored, next_third_least_explored = self.decode_state(next_state)
                ...
                # self.Q[walls[0], walls[1], walls[2], walls[3], borders[0], borders[1], borders[2], borders[3], dead_ends[0], dead_ends[1], dead_ends[2], dead_ends[3], target[0], target[1], target[2], target[3], least_explored, second_least_explored, third_least_explored, action] += alpha * (reward + gamma * np.max(self.Q[next_walls[0], next_walls[1], next_walls[2], next_walls[3], next_borders[0], next_borders[1], next_borders[2], next_borders[3], next_dead_ends[0], next_dead_ends[1], next_dead_ends[2], next_dead_ends[3], next_target[0], next_target[1], next_target[2], next_target[3], next_least_explored, next_second_least_explored, next_third_least_explored]) - self.Q[walls[0], walls[1], walls[2], walls[3], borders[0], borders[1], borders[2], borders[3], dead_ends[0], dead_ends[1], dead_ends[2], dead_ends[3], target[0], target[1], target[2], target[3], least_explored, second_least_explored, third_least_explored, action])
                state = next_state
                if i > limit:
                    break
                i += 1
            if k % 100 == 0:
                print("Iteration", k)
        try:
            np.save('Q_matrices/16h_train_advanced.npy', self.Q)
        except:
            print("Could not save Q matrix")
        print("Learning done!")
